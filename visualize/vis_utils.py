...
def project_to_image(pts_3d, P):
    ...
    pts_2d = np.dot(pts_3d_extend, np.transpose(P))  # (8, 4) /dot (4, 3) -> (8, 3)
    # normalize image coordinations to pixel coordination.
    pts_2d[:, 0] /= pts_2d[:, 2] 
    pts_2d[:, 1] /= pts_2d[:, 2]
    return pts_2d[:, 0:2]

def calc_rotx(norm_vec):
    cos_value = np.sum(norm_vec * np.array([0, 1, 0], dtype=np.float32)) \
                    / np.linalg.norm(norm_vec)
    return np.pi - np.arccos(cos_value)

def calc_roty(norm_vec):
    cos_value = np.sum(norm_vec * np.array([0, 0, 0], dtype=np.float32)) \
                    / np.linalg.norm(norm_vec)
    return np.pi - np.arccos(cos_value)

def calc_rotz(norm_vec):
    cos_value = np.sum(norm_vec * np.array([1, 0, 0], dtype=np.float32)) \
                    / np.linalg.norm(norm_vec)
